...
(function () {
  ...
  // ---------- Helpers de script dinâmico (chat/css/socket) ----------
  ...
  // Guard p/ não inicializar o chat duas vezes
  ...
  async function bootChat() {
    ...
    try {
      // 1) CSS do chat (garante independentemente do header)
      ensureLinkOnce('/assets/css/sb-chat.css');

      // 2) Socket.IO precisa existir antes do connectSocketIO() do chat.js
      await injectScriptOnce('/socket.io/socket.io.js');

      // 3) chat.js (caminho raiz e fallback)
      if (!window.initChat) {
  ...
  window.carregarHeader = async function (containerId = 'header-placeholder', user = null) {
    ...
    try {
      ...
      // Carrinho (oculto em páginas de auth)
      if (isAuthPage()) {
      ...
      // Chat sempre que o header carregar
      bootChat();
